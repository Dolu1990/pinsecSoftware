...
#define N 512+17			/* Number of dots */
#define SCALE 8192
...
int16_t *cosi = &sine[SCALE/4]; /* cosf(x) = sinf(x+90d)... */
...
void initialize (void)
{
  ...
  for (i = 0; i < SCALE+(SCALE/4); i++){
	...
	float s = sinf(PI2 * i / SCALE);
	//printf("alpha=%f sinf=%f\n",360.0f*i/SCALE,s);
    sine[i] = (int)(s* SCALE);
  }
}
  ...
  for (i = 0; i < N; i++)
    {
      ...
      projX = width / 2 + (xyz[0][i] * projZ / SCALE) / 36;
      projY = height / 2 + (xyz[1][i] * projZ / SCALE) / 36;
